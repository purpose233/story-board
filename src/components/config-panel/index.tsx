...
const generateVisuals = (elementConfig: IEditorElementConfig, editorStore: EditorStore) => {
        ...
        case 'string':
          return (
            <div className="config-panel-entry" key={visual.channel}>
              ...
              <Input defaultValue={value} onChange={onChange} style={{ width: 180 }}></Input>
            </div>
          );
        case 'number':
          return (
            <div className="config-panel-entry" key={visual.channel}>
              <span className="config-panel-entry-label">{visual.channel}:</span>
              <InputNumber defaultValue={value} onChange={onChange} style={{ width: 180 }} />
            </div>
          );
        case 'color':
          return (
...
export const ConfigPanel = observer(() => {
  ...
  return (
    <Card title="Config Panel" style={{ width: '100%', height: '100%' }}>
      {editorStore.currentElement ? generateVisuals(editorStore.currentElement, editorStore) : null}
    </Card>
  );
});
